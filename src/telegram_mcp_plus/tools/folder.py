...
def _get_dialog_type(entity) -> str:
...
async def tg_folder(folder_id: int, only_unread: bool = False, offset: Optional[str] = None) -> dict:
    """Get dialogs from a specific Telegram folder."""
    try:
        client = await ensure_connected()

        kwargs = {"folder": folder_id, "limit": 20}
        ...
        error_msg = str(e)
        if "folder" in error_msg.lower() or "filter" in error_msg.lower():
            return {"error": f"Folder not found: {folder_id}"}
        return {"error": f"Folder not found: {folder_id}"}
